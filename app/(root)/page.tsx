// app/(root)/page.tsx
// Server component — reads session server-side so buttons reflect login state
...
export default async function HomePage() {
  const [session, { jobs }, categories] = await Promise.all([
    auth(),
    getJobs({ page: 1, limit: 3, sort: "newest", status: "OPEN" }),
    getCategories(),
  ]);
        ...
        <section className="relative overflow-hidden py-20 md:py-32">
          ...
          <div className="container mx-auto px-4 text-center relative">
            <div className="inline-flex items-center gap-2 bg-brand/10 border border-brand/20 rounded-full px-4 py-1.5 text-sm text-brand mb-6">
              ...
              New accounting jobs posted daily
            </div>

            <h1 className="text-4xl md:text-6xl font-bold mb-6 tracking-tight">
              Find Accounting Jobs.<br />
              <span className="text-brand">Win More Clients.</span>
            </h1>

            <p className="text-lg text-muted-foreground max-w-xl mx-auto mb-8">
              The specialized marketplace connecting skilled accountants with companies that need their expertise.
            </p>
        ...
        <section className="py-16 border-y border-border">
          <div className="container mx-auto px-4">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              {[
                { icon: Briefcase, title: "Specialized Jobs", desc: "Tax, audit, bookkeeping, payroll — matched to your expertise." },
                { icon: Shield, title: "Secure Bidding", desc: "Submit proposals confidently with our protected platform." },
                { icon: Zap, title: "Fast Matching", desc: "Get in front of clients faster with smart filtering and search." },
              ].map((f) => (
          ...
          <div className="container mx-auto px-4">
            ...
            <div className="flex flex-wrap gap-3 justify-center">
              {categories.map((cat) => (
                ...
                >
                  <span className="w-2.5 h-2.5 rounded-full" style={{ background: cat.color ?? "#019a51" }} />
                  <span className="text-sm font-medium group-hover:text-brand transition-colors">{cat.name}</span>
                  <span className="text-xs text-muted-foreground">({cat._count?.jobs ?? 0})</span>
                </Link>
              ))}
            </div>
          </div>
            ...
            <div className="container mx-auto px-4">
              <div className="flex items-center justify-between mb-8">
                <div>
                  ...
                  <p className="text-muted-foreground text-sm mt-1">Fresh accounting jobs posted recently</p>
                </div>
                <Link href="/jobs">
          ...
          <div className="container mx-auto px-4 text-center">
            <div className="max-w-lg mx-auto bg-card border border-border rounded-2xl p-10">
              ...
              <p className="text-muted-foreground text-sm mb-6">
                Join accountants already winning clients on Accountant Hub.
              </p>
              {isLoggedIn ? (
                <Link href="/jobs">
